...
class CronParser
{
    ...
    {
        $this->_now = strtotime($date);
    }

    public function hasRunInTheLast($seconds)
    {
        // floor to nearest minute
        $tmp = date('Y-m-d H:i:00', $this->_now - $seconds);
        $current = strtotime($tmp);

        while ($current <= $this->_now)
        {
